...
Una **libreria di feed RSS multilingue** in formato OPML. Usala in qualsiasi lettore RSS abbonandoti al singolo URL OPML.

## URL del feed

Usa questo URL nel tuo client RSS/OPML:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Scopo

- Feed RSS selezionati e raggruppati per categoria (scienza, tecnologia, notizie, intrattenimento, cultura, sport, economia, ecc.).
- Supporto per più lingue (es. turco, inglese).
- Un solo URL per l’elenco completo dei feed; non serve aggiungere i feed uno per uno.

## Contribuire

Nuovi feed e categorie sono benvenuti. Apri una **Pull Request** per proporre aggiunte o modifiche.

- Aggiungi i feed nella categoria e nella sezione lingua corretta in `feeds.opml`.
- Mantieni la struttura OPML e i nomi degli attributi esistenti (`text`, `title`, `xmlUrl`, `type="rss"`).

## Esempio di struttura di un feed

Ogni voce di feed in `feeds.opml` usa gli stessi attributi outline OPML. Di seguito 10 righe di esempio da categorie e lingue diverse.

| Lingua | Categoria | Tipo | Titolo | Testo | URL |
|--------|-----------|------|--------|-------|-----|
...
In XML OPML, un feed è un singolo elemento `<outline>`, ad es.:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Lingua** — Raggruppamento di primo livello (`outline language="tr"` o `language="en"`).
- **Categoria** — `text`/`title` dell’outline padre (es. `baseFeed.science`, `baseFeed.technology`).
- **Tipo** — Per le voci feed è sempre `rss`.
- **Titolo** / **Testo** — Nome visualizzato del feed (spesso uguale).
- **URL** — URL del feed nell’attributo `xmlUrl`.

## Repository

- **feeds.opml** — File OPML principale (servito all’URL del feed sopra).
- **scripts/** — Utility (es. validazione feed).
- **.github/workflows/** — Automazione (es. controlli feed).

## Licenza

Questo progetto è gratuito e open source. Vedi [LICENSE](../LICENSE) per i dettagli.

...
Una **biblioteca de canals RSS multilingüe** en format OPML. Utilitzeu-la en qualsevol lector RSS subscrivint-vos a la única URL OPML.

## URL del canal

Utilitzeu aquesta URL al vostre client RSS/OPML:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Propòsit

- Canals RSS seleccionats i agrupats per categoria (ciència, tecnologia, notícies, entreteniment, cultura, esports, economia, etc.).
- Múltiples idiomes (p. ex. turc, anglès).
- Una sola URL per obtenir la llista completa; no cal afegir canals un per un.

## Contribuir

Nous canals i categories són benvinguts. Obriu un **Pull Request** per afegir o proposar canvis.

- Afegiu canals a la categoria i secció d’idioma correctes a `feeds.opml`.
- Manteniu l’estructura OPML i els noms d’atributs (`text`, `title`, `xmlUrl`, `type="rss"`).

## Estructura d’exemple d’un canal

Cada entrada de canal a `feeds.opml` fa servir els mateixos atributs outline d’OPML. A sota hi ha 10 files d’exemple de categories i idiomes diferents.

| Idioma | Categoria | Tipus | Títol | Text | URL |
|--------|-----------|-------|-------|------|-----|
...
En XML OPML, un canal és un sol element `<outline>`, per exemple:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Idioma** — Agrupació de primer nivell (`outline language="tr"` o `language="en"`).
- **Categoria** — `text`/`title` de l’outline pare (p. ex. `baseFeed.science`, `baseFeed.technology`).
- **Tipus** — Sempre `rss` per a entrades de canal.
- **Títol** / **Text** — Nom visible del canal (sovint igual).
- **URL** — URL del canal a l’atribut `xmlUrl`.

## Repositori

- **feeds.opml** — Fitxer OPML principal (servit a la URL anterior).
- **scripts/** — Utilitats (p. ex. validació de canals).
- **.github/workflows/** — Automatització (p. ex. comprovacions de canals).

## Llicència

Aquest projecte és lliure i de codi obert. Vegeu [LICENSE](../LICENSE) per als detalls.

---

## README en altres idiomes

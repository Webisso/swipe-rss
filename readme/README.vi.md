...
**Thư viện nguồn cấp RSS đa ngôn ngữ** ở định dạng OPML. Dùng trong bất kỳ trình đọc RSS nào bằng cách đăng ký một URL OPML duy nhất.

## URL nguồn cấp

Dùng URL này trong ứng dụng khách RSS/OPML của bạn:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Mục đích

- Các nguồn cấp RSS được chọn lọc, nhóm theo danh mục (khoa học, công nghệ, tin tức, giải trí, văn hóa, thể thao, kinh tế, v.v.).
- Hỗ trợ nhiều ngôn ngữ (ví dụ: Thổ Nhĩ Kỳ, Anh).
- Một URL cho toàn bộ danh sách nguồn cấp; không cần thêm từng nguồn một.

## Đóng góp

Nguồn cấp và danh mục mới đều được chào đón. Hãy mở **Pull Request** để thêm hoặc đề xuất thay đổi.

- Thêm nguồn cấp vào đúng danh mục và phần ngôn ngữ trong `feeds.opml`.
- Giữ nguyên cấu trúc OPML và tên thuộc tính hiện có (`text`, `title`, `xmlUrl`, `type="rss"`).

## Ví dụ cấu trúc nguồn cấp

Mỗi mục nguồn cấp trong `feeds.opml` dùng cùng các thuộc tính outline OPML. Dưới đây là 10 dòng ví dụ từ các danh mục và ngôn ngữ khác nhau.

| Ngôn ngữ | Danh mục | Loại | Tiêu đề | Văn bản | URL |
|----------|-----------|------|---------|---------|-----|
...
Trong OPML XML, một nguồn cấp là một phần tử `<outline>`, ví dụ:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Ngôn ngữ** — Nhóm cấp cao nhất (`outline language="tr"` hoặc `language="en"`).
- **Danh mục** — `text`/`title` của outline cha (ví dụ `baseFeed.science`, `baseFeed.technology`).
- **Loại** — Với mục nguồn cấp luôn là `rss`.
- **Tiêu đề** / **Văn bản** — Tên hiển thị của nguồn cấp (thường giống nhau).
- **URL** — URL nguồn cấp trong thuộc tính `xmlUrl`.

## Kho mã

- **feeds.opml** — Tệp OPML chính (phục vụ tại URL nguồn cấp trên).
- **scripts/** — Tiện ích (ví dụ: kiểm tra nguồn cấp).
- **.github/workflows/** — Tự động hóa (ví dụ: kiểm tra nguồn cấp).

## Giấy phép

Dự án này miễn phí và mã nguồn mở. Xem [LICENSE](../LICENSE) để biết chi tiết.

---

## README bằng ngôn ngữ khác

...
Ett **flerspråkigt RSS-flödesbibliotek** i OPML-format. Använd det i valfri RSS-läsare genom att prenumerera på den enda OPML-URL:en.

## Flödes-URL

Använd denna URL i din RSS/OPML-klient:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Syfte

- Kuraterade RSS-flöden grupperade efter kategori (vetenskap, teknik, nyheter, underhållning, kultur, sport, ekonomi m.m.).
- Flera språk stöds (t.ex. turkiska, engelska).
- En URL för hela flödeslistan; behöver inte lägga till flöden ett och ett.

## Bidra

Nya flöden och kategorier är välkomna. Öppna en **Pull Request** för att lägga till eller föreslå ändringar.

- Lägg till flöden i rätt kategori och språksektion i `feeds.opml`.
- Behåll den befintliga OPML-strukturen och attributnamnen (`text`, `title`, `xmlUrl`, `type="rss"`).

## Exempel på flödesstruktur

Varje flödespost i `feeds.opml` använder samma OPML outline-attribut. Nedan 10 exempelrader från olika kategorier och språk.

| Språk | Kategori | Typ | Titel | Text | URL |
|-------|-----------|-----|-------|------|-----|
...
I OPML XML är ett flöde ett `<outline>`-element, t.ex.:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Språk** — Gruppering på översta nivån (`outline language="tr"` eller `language="en"`).
- **Kategori** — Överordnad outlines `text`/`title` (t.ex. `baseFeed.science`, `baseFeed.technology`).
- **Typ** — För flödesposter alltid `rss`.
- **Titel** / **Text** — Visningsnamn för flödet (ofta samma).
- **URL** — Flödes-URL i attributet `xmlUrl`.

## Arkiv

- **feeds.opml** — Huvud-OPML-fil (serveras på flödes-URL:en ovan).
- **scripts/** — Verktyg (t.ex. flödesvalidering).
- **.github/workflows/** — Automatisering (t.ex. flödeskontroller).

## Licens

Detta projekt är gratis och öppen källkod. Se [LICENSE](../LICENSE) för detaljer.

---

## README på andra språk

...
Egy **többnyelvű RSS feed könyvtár** OPML formátumban. Bármely RSS olvasóban használható egyetlen OPML URL feliratkozással.

## Feed URL

Használd ezt az URL-t az RSS/OPML kliensedben:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Cél

- Kurált RSS feedek kategóriánként (tudomány, technológia, hírek, szórakozás, kultúra, sport, gazdaság stb.).
- Több nyelv támogatva (pl. török, angol).
- Egy URL a teljes feedlistához; nem kell egyenként hozzáadni a feedeket.

## Közreműködés

Új feedek és kategóriák szívesen látottak. Nyiss **Pull Request**-et a hozzáadáshoz vagy módosítások javaslatához.

- Add hozzá a feedeket a megfelelő kategóriához és nyelvi szekcióhoz a `feeds.opml` fájlban.
- Tartsd meg a meglévő OPML szerkezetet és attribútumneveket (`text`, `title`, `xmlUrl`, `type="rss"`).

## Példa feed szerkezetre

A `feeds.opml` minden feed bejegyzése ugyanazokat az OPML outline attribútumokat használja. Alább 10 példasor különböző kategóriákból és nyelvekből.

| Nyelv | Kategória | Típus | Cím | Szöveg | URL |
|-------|------------|-------|-----|--------|-----|
...
OPML XML-ben egy feed egy `<outline>` elem, pl.:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Nyelv** — Legfelső szintű csoportosítás (`outline language="tr"` vagy `language="en"`).
- **Kategória** — Szülő outline `text`/`title` (pl. `baseFeed.science`, `baseFeed.technology`).
- **Típus** — Feed bejegyzéseknél mindig `rss`.
- **Cím** / **Szöveg** — A feed megjelenített neve (gyakran azonos).
- **URL** — A feed URL-je az `xmlUrl` attribútumban.

## Tárház

- **feeds.opml** — Fő OPML fájl (a fenti feed URL-en érhető el).
- **scripts/** — Segédprogramok (pl. feed ellenőrzés).
- **.github/workflows/** — Automatizálás (pl. feed ellenőrzések).

## Licenc

Ez a projekt ingyenes és nyílt forráskódú. Részletek: [LICENSE](../LICENSE).

---

## README más nyelveken

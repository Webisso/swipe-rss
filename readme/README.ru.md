...
**Многоязычная библиотека RSS-лент** в формате OPML. Используйте её в любом RSS-читателе, подписавшись на один URL OPML.

## URL ленты

Используйте этот URL в клиенте RSS/OPML:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Цель

- Отобранные RSS-ленты, сгруппированные по категориям (наука, технологии, новости, развлечения, культура, спорт, экономика и т.д.).
- Поддержка нескольких языков (например, турецкий, английский).
- Один URL для полного списка лент; не нужно добавлять ленты по одной.

## Участие в разработке

Новые ленты и категории приветствуются. Откройте **Pull Request**, чтобы добавить или предложить изменения.

- Добавляйте ленты в нужную категорию и языковой раздел в `feeds.opml`.
- Сохраняйте существующую структуру OPML и имена атрибутов (`text`, `title`, `xmlUrl`, `type="rss"`).

## Пример структуры ленты

Каждая запись ленты в `feeds.opml` использует одни и те же атрибуты outline OPML. Ниже 10 примеров строк из разных категорий и языков.

| Язык | Категория | Тип | Заголовок | Текст | URL |
|------|-----------|-----|-----------|-------|-----|
...
В OPML XML одна лента — это один элемент `<outline>`, например:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Язык** — Группировка верхнего уровня (`outline language="tr"` или `language="en"`).
- **Категория** — `text`/`title` родительского outline (напр. `baseFeed.science`, `baseFeed.technology`).
- **Тип** — Для записей лент всегда `rss`.
- **Заголовок** / **Текст** — Отображаемое имя ленты (часто совпадают).
- **URL** — URL ленты в атрибуте `xmlUrl`.

## Репозиторий

- **feeds.opml** — Основной файл OPML (доступен по URL ленты выше).
- **scripts/** — Утилиты (напр. проверка лент).
- **.github/workflows/** — Автоматизация (напр. проверки лент).

## Лицензия

Проект бесплатный и с открытым исходным кодом. Подробности в [LICENSE](../LICENSE).

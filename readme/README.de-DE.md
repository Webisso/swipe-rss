...
Eine **mehrsprachige RSS-Feed-Bibliothek** im OPML-Format. Nutze sie in jedem RSS-Reader, indem du die eine OPML-URL abonnierst.

## Feed-URL

Diese URL in deinem RSS-/OPML-Client verwenden:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Zweck

- Kuratierte RSS-Feeds, nach Kategorie gruppiert (Wissenschaft, Technik, Nachrichten, Unterhaltung, Kultur, Sport, Wirtschaft usw.).
- Mehrere Sprachen (z. B. Türkisch, Englisch).
- Eine URL für die komplette Feed-Liste; Feeds müssen nicht einzeln hinzugefügt werden.

## Mitmachen

Neue Feeds und Kategorien sind willkommen. Bitte öffne einen **Pull Request**, um etwas hinzuzufügen oder Änderungen vorzuschlagen.

- Feeds in der richtigen Kategorie und Sprach-Sektion in `feeds.opml` eintragen.
- Die bestehende OPML-Struktur und Attributnamen beibehalten (`text`, `title`, `xmlUrl`, `type="rss"`).

## Beispiel-Feed-Struktur

Jeder Feed-Eintrag in `feeds.opml` nutzt dieselben OPML-Outline-Attribute. Unten stehen 10 Beispielzeilen aus verschiedenen Kategorien und Sprachen.

| Sprache | Kategorie | Typ | Titel | Text | URL |
|---------|------------|-----|-------|------|-----|
...
In OPML-XML ist ein Feed ein `<outline>`-Element, z. B.:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Sprache** — Gruppierung auf oberster Ebene (`outline language="tr"` oder `language="en"`).
- **Kategorie** — `text`/`title` des übergeordneten Outline (z. B. `baseFeed.science`, `baseFeed.technology`).
- **Typ** — Bei Feed-Einträgen immer `rss`.
- **Titel** / **Text** — Anzeigename des Feeds (oft identisch).
- **URL** — Feed-URL im Attribut `xmlUrl`.

## Repository

- **feeds.opml** — Haupt-OPML-Datei (unter der Feed-URL oben erreichbar).
- **scripts/** — Hilfsprogramme (z. B. Feed-Validierung).
- **.github/workflows/** — Automatisierung (z. B. Feed-Prüfungen).

## Lizenz

Dieses Projekt ist kostenlos und Open Source. Details siehe [LICENSE](../LICENSE).

...
**مكتبة موجزات RSS متعددة اللغات** بتنسيق OPML. استخدمها في أي قارئ RSS بالاشتراك في عنوان OPML الواحد.

## عنوان الموجز

استخدم هذا الرابط في عميل RSS/OPML:

**https://webisso.github.io/swipe-rss/feeds.opml**

## الغرض

- موجزات RSS منتقاة مصنفة حسب الفئة (العلوم، التقنية، الأخبار، الترفيه، الثقافة، الرياضة، الاقتصاد، إلخ).
- دعم لغات متعددة (مثل التركية، الإنجليزية).
- عنوان واحد للحصول على قائمة الموجزات كاملة؛ لا حاجة لإضافة الموجزات واحداً تلو الآخر.

## المساهمة

الموجزات والفئات الجديدة مرحب بها. يرجى فتح **طلب سحب (Pull Request)** للإضافة أو اقتراح تغييرات.

- أضف الموجزات في القسم المناسب من الفئة واللغة في `feeds.opml`.
- حافظ على بنية OPML وأسماء السمات الحالية (`text`, `title`, `xmlUrl`, `type="rss"`).

## هيكل موجز مثال

كل إدخال موجز في `feeds.opml` يستخدم نفس سمات outline في OPML. فيما يلي 10 صفوف مثال من فئات ولغات مختلفة.

| اللغة | الفئة | النوع | العنوان | النص | الرابط |
|-------|-------|------|---------|------|--------|
...
في OPML XML، كل موجز واحد هو عنصر `<outline>` واحد، مثلاً:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **اللغة** — التجميع على المستوى الأعلى (`outline language="tr"` أو `language="en"`).
- **الفئة** — النص/العنوان في outline الأب (مثل `baseFeed.science`, `baseFeed.technology`).
- **النوع** — دائماً `rss` لإدخالات الموجز.
- **العنوان** / **النص** — الاسم المعروض للموجز (غالباً متطابق).
- **الرابط** — رابط الموجز في السمة `xmlUrl`.

## المستودع

- **feeds.opml** — ملف OPML الرئيسي (يُقدّم على عنوان الموجز أعلاه).
- **scripts/** — أدوات مساعدة (مثل التحقق من الموجزات).
- **.github/workflows/** — أتمتة (مثل فحوص الموجزات).

## الترخيص

هذا المشروع مجاني ومفتوح المصدر. للتفاصيل انظر [LICENSE](../LICENSE).

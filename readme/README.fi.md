...
**Monikielinen RSS-syötekirjasto** OPML-muodossa. Käytä sitä millä tahansa RSS-lukijalla tilaamalla yhden OPML-URL:n.

## Syötteen URL

Käytä tätä URL-osoitetta RSS/OPML-asiakassovelluksessasi:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Tarkoitus

- Valikoidut RSS-syötteet kategorioittain (tiede, teknologia, uutiset, viihde, kulttuuri, urheilu, talous jne.).
- Useita kieliä tuettu (esim. turkki, englanti).
- Yksi URL koko syöteluetteloon; syötteitä ei tarvitse lisätä yksi kerrallaan.

## Osallistuminen

Uudet syötteet ja kategoriat ovat tervetulleita. Avaa **Pull Request** lisätäksesi tai ehdottaaksesi muutoksia.

- Lisää syötteet oikeaan kategoriaan ja kieliosioon tiedostossa `feeds.opml`.
- Säilytä nykyinen OPML-rakenne ja attribuuttien nimet (`text`, `title`, `xmlUrl`, `type="rss"`).

## Esimerkki syötteen rakenteesta

Jokainen syötemerkinne `feeds.opml`-tiedostossa käyttää samoja OPML outline -attribuutteja. Alla 10 esimerkkiriviä eri kategorioista ja kielistä.

| Kieli | Kategoria | Tyyppi | Otsikko | Teksti | URL |
|-------|-----------|--------|---------|--------|-----|
...
OPML XML:ssä yksi syöte on yksi `<outline>`-elementti, esim.:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Kieli** — Ylimmän tason ryhmitys (`outline language="tr"` tai `language="en"`).
- **Kategoria** — Ylätason outlinen `text`/`title` (esim. `baseFeed.science`, `baseFeed.technology`).
- **Tyyppi** — Syötemerkinneille aina `rss`.
- **Otsikko** / **Teksti** — Syötteen näyttönimi (usein sama).
- **URL** — Syötteen URL `xmlUrl`-attribuutissa.

## Repositorio

- **feeds.opml** — Pää OPML-tiedosto (tarjolla yllä olevalla syöte-URL:lla).
- **scripts/** — Apuohjelmat (esim. syötteen validointi).
- **.github/workflows/** — Automatisointi (esim. syötetarkistukset).

## Lisenssi

Tämä projekti on ilmainen ja avoimen lähdekoodin. Tiedot [LICENSE](../LICENSE).

---

## README muilla kielillä

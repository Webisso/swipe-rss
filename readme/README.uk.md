...
**Багатомовна бібліотека RSS-стрічок** у форматі OPML. Використовуйте її в будь-якому RSS-читачі, підписавшись на одну OPML-адресу.

## URL стрічки

Використовуйте цю адресу в клієнті RSS/OPML:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Мета

- Відібрані RSS-стрічки, згруповані за категоріями (наука, технології, новини, розваги, культура, спорт, економіка тощо).
- Підтримка кількох мов (наприклад, турецька, англійська).
- Одна адреса для повного списку стрічок; не потрібно додавати стрічки по одній.

## Участь

Нові стрічки та категорії вітаються. Відкрийте **Pull Request**, щоб додати або запропонувати зміни.

- Додавайте стрічки до відповідної категорії та мовної секції в `feeds.opml`.
- Зберігайте існуючу структуру OPML та назви атрибутів (`text`, `title`, `xmlUrl`, `type="rss"`).

## Приклад структури стрічки

Кожен запис стрічки в `feeds.opml` використовує ті самі атрибути outline OPML. Нижче 10 прикладів рядків з різних категорій та мов.

| Мова | Категорія | Тип | Заголовок | Текст | URL |
|------|-----------|-----|-----------|-------|-----|
...
В OPML XML одна стрічка — це один елемент `<outline>`, наприклад:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Мова** — Групування верхнього рівня (`outline language="tr"` або `language="en"`).
- **Категорія** — `text`/`title` батьківського outline (напр. `baseFeed.science`, `baseFeed.technology`).
- **Тип** — Для записів стрічок завжди `rss`.
- **Заголовок** / **Текст** — Відображувана назва стрічки (часто однакові).
- **URL** — Адреса стрічки в атрибуті `xmlUrl`.

## Сховище

- **feeds.opml** — Основний файл OPML (доступний за адресою стрічки вище).
- **scripts/** — Утиліти (напр. перевірка стрічок).
- **.github/workflows/** — Автоматизація (напр. перевірки стрічок).

## Ліцензія

Цей проєкт безкоштовний і з відкритим вихідним кодом. Деталі в [LICENSE](../LICENSE).

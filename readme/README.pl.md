...
**Wielojęzyczna biblioteka kanałów RSS** w formacie OPML. Użyj jej w dowolnym czytniku RSS, subskrybując jeden adres OPML.

## Adres kanału

Użyj tego adresu w kliencie RSS/OPML:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Cel

- Wyselekcjonowane kanały RSS pogrupowane według kategorii (nauka, technologia, wiadomości, rozrywka, kultura, sport, gospodarka itd.).
- Wsparcie wielu języków (np. turecki, angielski).
- Jeden adres do pełnej listy kanałów; nie trzeba dodawać kanałów pojedynczo.

## Wkład

Nowe kanały i kategorie są mile widziane. Otwórz **Pull Request**, aby dodać lub zaproponować zmiany.

- Dodawaj kanały we właściwej kategorii i sekcji językowej w pliku `feeds.opml`.
- Zachowaj istniejącą strukturę OPML i nazwy atrybutów (`text`, `title`, `xmlUrl`, `type="rss"`).

## Przykład struktury kanału

Każdy wpis kanału w `feeds.opml` używa tych samych atrybutów outline OPML. Poniżej 10 przykładowych wierszy z różnych kategorii i języków.

| Język | Kategoria | Typ | Tytuł | Tekst | URL |
|-------|-----------|-----|-------|-------|-----|
...
W OPML XML jeden kanał to jeden element `<outline>`, np.:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Język** — Grupowanie najwyższego poziomu (`outline language="tr"` lub `language="en"`).
- **Kategoria** — `text`/`title` outline nadrzędnego (np. `baseFeed.science`, `baseFeed.technology`).
- **Typ** — Dla wpisów kanału zawsze `rss`.
- **Tytuł** / **Tekst** — Nazwa wyświetlana kanału (często taka sama).
- **URL** — Adres kanału w atrybucie `xmlUrl`.

## Repozytorium

- **feeds.opml** — Główny plik OPML (udostępniany pod powyższym adresem kanału).
- **scripts/** — Narzędzia (np. walidacja kanałów).
- **.github/workflows/** — Automatyzacja (np. sprawdzanie kanałów).

## Licencja

Ten projekt jest bezpłatny i open source. Szczegóły w [LICENSE](../LICENSE).

...
OPML formatında **çok dilli bir RSS feed kütüphanesi**. Tek OPML adresine abone olarak herhangi bir RSS okuyucuda kullanabilirsiniz.

## Feed adresi

RSS/OPML istemcinizde şu adresi kullanın:

**https://webisso.github.io/swipe-rss/feeds.opml**

## Amaç

- Kategoriye göre gruplanmış, seçilmiş RSS feed’leri (bilim, teknoloji, haber, eğlence, kültür, spor, ekonomi vb.).
- Birden fazla dil desteği (ör. Türkçe, İngilizce).
- Tüm feed listesini almak için tek adres; feed’leri tek tek eklemenize gerek yok.

## Katkıda bulunma

Yeni feed’ler ve kategoriler memnuniyetle karşılanır. Eklemek veya değişiklik önermek için lütfen bir **Pull Request** açın.

- Feed’leri `feeds.opml` içinde doğru kategori ve dil bölümüne ekleyin.
- Mevcut OPML yapısını ve öznitelik adlarını (`text`, `title`, `xmlUrl`, `type="rss"`) koruyun.

## Örnek feed yapısı

`feeds.opml` içindeki her feed girişi aynı OPML outline özniteliklerini kullanır. Aşağıda farklı kategorilerden ve dillerden 10 örnek satır bulunmaktadır.

| Dil | Kategori | Tip | Başlık | Metin | URL |
|-----|----------|-----|--------|-------|-----|
...
OPML XML’de tek bir feed tek bir `<outline>` öğesidir, örnek:

```xml
<outline type="rss" text="Webrazzi" title="Webrazzi" xmlUrl="https://webrazzi.com/feed"/>
```

- **Dil** — Üst düzey gruplama (`outline language="tr"` veya `language="en"`).
- **Kategori** — Üst outline’ın `text`/`title` değeri (ör. `baseFeed.science`, `baseFeed.technology`).
- **Tip** — Feed girişleri için her zaman `rss`.
- **Başlık** / **Metin** — Feed’in görünen adı (genelde aynı).
- **URL** — `xmlUrl` özniteliğindeki feed adresi.

## Depo

- **feeds.opml** — Ana OPML dosyası (yukarıdaki feed adresinde sunulur).
- **scripts/** — Yardımcı araçlar (ör. feed doğrulama).
- **.github/workflows/** — Otomasyon (ör. feed kontrolleri).

## Lisans

Bu proje ücretsiz ve açık kaynaklıdır. Ayrıntılar için [LICENSE](../LICENSE) dosyasına bakın.
